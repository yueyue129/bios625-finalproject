...
find_optimal_threshold <- function(y_true, y_pred_proba, method = "youden") {
  ...
  if (method == "youden") {
    ...
    f1_scores <- sapply(thresholds, function(t) {
      ...
      cm <- confusionMatrix(factor(y_pred_binary, levels = c(0, 1)), 
...
log_pred_lognorm_train <- predict(severity_lognorm, 
                                   newdata = train_data %>% filter(any_cost == 1))
log_pred_lognorm_test <- predict(severity_lognorm, 
                                  newdata = test_data)
...
mu_gamma_train <- predict(severity_gamma, 
                          newdata = train_data %>% filter(any_cost == 1),
                          type = "response")
mu_gamma_test <- predict(severity_gamma, 
                         newdata = test_data,
                         type = "response")
...
write.csv(results_formatted, file = file.path(output_dir, "model_performance_v3.csv"), row.names = FALSE)
...
png(file.path(output_dir, "residuals", "residuals_scatter_v3.png"), width = 10, height = 6, units = "in", res = 300)
...
png(file.path(output_dir, "residuals", "qq_plot_v3.png"), width = 10, height = 6, units = "in", res = 300)
qqnorm(residuals, main = "Q-Q Plot of Residuals (V3)")
qqline(residuals, col = "red")
dev.off()

png(file.path(output_dir, "residuals", "residuals_histogram_v3.png"), width = 10, height = 6, units = "in", res = 300)
...
cv_results <- map_dfr(1:5, function(fold) {
  ...
  set.seed(625)
  dtrain_pos_cv <- xgb.DMatrix(data = X_train_cv_sev_ml_pos, label = y_train_pos_cv_trimmed)
  severity_xgb_cv <- xgboost(
    data = dtrain_pos_cv,
    nrounds = best_params_sev$nrounds,
    max_depth = best_params_sev$max_depth,
    eta = best_params_sev$eta,
    objective = "reg:squarederror",
    verbose = 0
  )
...
print(cv_summary)
write.csv(cv_results, file = file.path(output_dir, "cv_results_v3.csv"), row.names = FALSE)
write.csv(cv_summary, file = file.path(output_dir, "cv_summary_v3.csv"), row.names = FALSE)
...
png(file.path(output_dir, "calibration", "calibration_curve_v3.png"), width = 10, height = 6, units = "in", res = 300)
...
text(calibration_data$mean_predicted, calibration_data$mean_actual,
     labels = calibration_data$pred_decile, pos = 3, cex = 0.8)
dev.off()

png(file.path(output_dir, "calibration", "qq_calibration_v3.png"), width = 10, height = 6, units = "in", res = 300)
...
write.csv(calibration_data, file = file.path(output_dir, "calibration", "calibration_data_v3.csv"), row.names = FALSE)
...
p_freq_glm_no_chronic_test <- predict(freq_glm_no_chronic, newdata = test_data, type = "response")
...
set.seed(625)
dtrain_freq_no_chronic <- xgb.DMatrix(data = X_train_freq_ml_no_chronic, label = any_cost_train)
freq_xgb_no_chronic <- xgboost(
...
sensitivity_results <- data.frame(
  Model = c("GLM (with chronic_count)", "GLM (without chronic_count)",
...
write.csv(sensitivity_results, file = file.path(output_dir, "sensitivity_analysis_v3.csv"), row.names = FALSE)
...
saveRDS(test_predictions_v3, file = file.path(output_dir, "test_predictions_v3.rds"))
write.csv(test_predictions_v3, file = file.path(output_dir, "test_predictions_v3.csv"), row.names = FALSE)
